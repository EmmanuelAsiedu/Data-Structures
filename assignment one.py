...
b = -L
c = L**2/6
#Using the Almight formular the roots are;
...
#Question f
"""
Let the absolute value the bending moment array be AM 
Also let the minimum AM be m_Am
"""
AM = abs(M)
m_AM = min(AM)
"""
When the bending moment is m_AM, we get the expression x**2-Lx+(l**2/6)+(2*m_AM)/w=0
"""
...
#Question h
"""
Let the maximum bending moment be max_M and the minimum be mim_M
"""
#for the maximum
max_M = max(M)
"""
...
min_M = min(M)
"""
When the bending moment is min_M, we get the expression x**2-Lx+(L**2//6)+(2*min_M)/w=0
"""
a = 1
b = -L
c = (L**2//6)+(2*min_M)/w
